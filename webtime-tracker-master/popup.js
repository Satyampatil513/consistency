...
var backgroundJS = chrome.extension.getBackgroundPage(),
  ...
  countRanks = function (e, t) {
    var n;
    for (ranks[t] = { total: e.length, domains: {} }, n = 0; n < e.length; n++)
  ...
  screenshotUIShow = function () {
    return (
      ...
      chrome.permissions.contains({ origins: ["<all_urls>"] }, function (e) {
        if (
...
initialize(),
  ...
  addMultipleDelegatedEventListeners(
    ...
    function (e, t) {
      ...
      if ("click" === e.type && t.classList.contains("domain"))
        renderDomainStats({ contextEl: t, range: s, connectId: o, url: i });
      else {
  ...
  addMultipleDelegatedEventListeners(
    ...
    function (e, t) {
      ...
      if ("screenshot" !== n)
        ...
        document.querySelector("#pseudomodal").dataset.visibility = n;
        var a = htmlTemplates.querySelector(
            "#screenshot-info-instructions-list"
          ),
  ...
  addMultipleDelegatedEventListeners(
    ...
    function (e, t) {
      return (
        chrome.permissions.request({ origins: ["<all_urls>"] }, function (e) {
          chrome.runtime.lastError &&
            chrome.runtime.lastError.message &&
            dcl(
...
var restoreFromJson = function (e) {
    var t = document.querySelector("#pseudomodal .options-restore"),
      ...
      r = function () {
        (n.innerText = n.dataset.warning),
          ...
          (a.querySelector(".shifter").style.animationIterationCount =
  ...
  getDomainData = function (e) {
    ...
    for (
      ...
      (o = d[a]),
        ...
        (l[s] += i),
        y.push({ date: o, seconds: i });
    return (r = {
      days: { total: v, domain: h },
      timeValues: { min: c, max: u },
  ...
  getOverallData = function () {
    ...
    for (i in backgroundJS.domains)
      if (backgroundJS.domains.hasOwnProperty(i)) {
        ...
        for (r in d)
          d.hasOwnProperty(r) &&
            ((s[r] = s[r] || { seconds: 0 }), (s[r].seconds += d[r].seconds));
      }
    for (t = 0; t < l.length; t++)
      (r = l[t]),
        ...
        (c[a] += o),
        o > 0 && E++,
        h.push({ date: r, seconds: o });
    return (n = {
      days: { total: v, visited: E },
      timeValues: {
        min: u === Number.MAX_SAFE_INTEGER ? 0 : u,
        max: m === Number.MIN_SAFE_INTEGER ? 0 : m,
      },
      visits: { first: S, last: y },
      times:
        ((e = {}),
  ...
  function getTimeForUrlToday(url) {
    var today = backgroundJS.dates.today;
    var domains = backgroundJS.domains;
  
    if (domains.hasOwnProperty(url) && domains[url].days.hasOwnProperty(today)) {
      return domains[url].days[today].seconds;
    }
  
    return 0; // Return 0 if no data is available for the URL on the current day
  }