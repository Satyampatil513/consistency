...
var dclCounter = 0,
  ...
  function getTimeForPresentDay(url) {
    if (typeof e === 'undefined') {
      console.error('Data object "e" is not available.');
      return null;
    }
  
    var domainData = e[url];
    if (!domainData) {
      console.error('URL not found in the data.');
      return null;
    }
  
    var totalSeconds = domainData.alltime.seconds;
    var presentDate = getDateString();
    var presentDayData = domainData.days[presentDate];
  
    if (!presentDayData) {
      console.error('No data available for the present day.');
      return null;
    }
  
    var seconds = presentDayData.seconds;
  
    var presentDayTime = {
      url: url,
      totalSeconds: totalSeconds,
      presentDaySeconds: seconds,
    };
  
    return presentDayTime;
  }
  
  // var presentDayTime = getTimeForPresentDay('www.youtube.com');
  // if (presentDayTime) {
  //   console.log(presentDayTime);
  // } else {
  //   console.log("No data available for the present day or invalid URL.");
  // }