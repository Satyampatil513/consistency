...
// Important Code
async function datatransfer(tabsData) {
    ...
    var target2=localStorage.getItem('target2');
    var target3=localStorage.getItem('target3');
    if(walletAddress){
        const dataToSend = [];
        for (let i = 0; i < tabsData.length; i++) {
        const y = tabsData[i].days.length - 1;
        const data = {
            domain: tabsData[i].url,
            date: new Date(tabsData[i].days[y].date).toLocaleDateString(),
            time: tabsData[i].days[y].summary,
            status: false
        };
        ...
        if(data.domain==url2){
            if(data.time>=target2){
                sendnotification(url2);
                data.status = true;
            }
            dataToSend.push(data);
        }
        if(data.domain==url3 ){
            if(data.time>=target3){
                sendnotification(url3);
                data.status = true;
            }
            dataToSend.push(data);
        }
        ...
        try {
            ...
            const response = await fetch(`http://localhost:3001/receive-data/${userId}`, {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            console.error('Failed to transfer data.');
        }
        } catch (error) {
...
chrome.runtime.onInstalled.addListener(function() {
    // Set up an alarm to trigger every minute (adjust the periodInMinutes as needed)
...
chrome.alarms.onAlarm.addListener(function(alarm) {
  ...
  function sendnotification(domain) {
    const notificationOptions = {
      type: 'basic',
      title: 'Dayly Dash Target Achieved',
      message: `Time to check ${domain}!`, // You can customize the message here
      iconUrl: '../icons/16x16.png', // Provide the URL to your icon image
